...
> Absorbed into `data-pipeline-review` in Skillry 3.0.0; formerly the standalone skill `model-serving-and-inference`.
> Origin: Skillry original, former path `plugins/data-ml-ai-engineering/skills/314-model-serving-and-inference/SKILL.md`.
...
- The concern is how the model was trained (reproducibility/leakage) — use `references/ml-training-pipeline-review.md`.
- The concern is the upstream feature ETL only — use the main `SKILL.md` procedure (data-pipeline-review).
