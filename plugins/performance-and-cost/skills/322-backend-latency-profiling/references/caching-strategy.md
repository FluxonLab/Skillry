> **Merged skill:** `caching-strategy` is part of `backend-latency-profiling` since Skillry 3.0.0 (routing table in `SKILL.md`).
> **Source:** Skillry original, formerly `plugins/performance-and-cost/skills/323-caching-strategy/SKILL.md`.
> **Changes:** YAML frontmatter replaced by this header; body unchanged.
> **Former description:** Use when you need to design or review a caching strategy across layers (CDN/edge/application/database), choose TTLs and cache keys, prevent cache stampedes, handle invalidation correctly, and measure hit rate to prove the cache is actually helping.
