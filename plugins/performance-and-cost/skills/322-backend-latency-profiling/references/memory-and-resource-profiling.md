> **Merged skill:** `memory-and-resource-profiling` is part of `backend-latency-profiling` since Skillry 3.0.0 (routing table in `SKILL.md`).
> **Source:** Skillry original, formerly `plugins/performance-and-cost/skills/324-memory-and-resource-profiling/SKILL.md`.
> **Changes:** YAML frontmatter replaced by this header; self-reference to the canonical skill re-pointed to SKILL.md.
> **Former description:** Use when you need to profile memory and OS resources — heap profiling, finding leaks and unbounded growth, GC pressure, and leaked file descriptors, sockets, or connections, including processes that hit container memory limits and get OOM-killed.
...
- The real issue is CPU/latency with flat memory — use the main backend latency profiling procedure in SKILL.md instead.
