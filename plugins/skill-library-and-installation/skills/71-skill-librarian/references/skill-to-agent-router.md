...
> Absorbed into `skill-librarian` in Skillry 3.0.0; formerly the standalone skill `skill-to-agent-router`.
> Origin: Skillry original, former path `plugins/skill-library-and-installation/skills/73-skill-to-agent-router/SKILL.md`.
...
- The task is to find which skill to use, not which agent — use the main `SKILL.md` procedure (skill-librarian).
...
- **Then** `architect` with `03-architecture-review` (its ADR reference, `references/adr-generator.md`) consumes the PRD's storage requirement to produce the ADR (step 3).
