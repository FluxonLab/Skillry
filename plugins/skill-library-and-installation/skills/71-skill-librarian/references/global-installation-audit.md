...
> Absorbed into `skill-librarian` in Skillry 3.0.0; formerly the standalone skill `global-installation-audit`.
> Origin: Skillry original, former path `plugins/skill-library-and-installation/skills/74-global-installation-audit/SKILL.md`.
...
- The goal is to find path placement errors across the filesystem — use `references/path-hygiene-repair.md`.
- The goal is to find duplicate skills — use `references/skill-deduplication.md`.
...
- **claude-system.toml paths pointing to wrong username**: `/Users//` in config when actual home is `$HOME/` — use `references/path-hygiene-repair.md` to fix.
...
- If the audit finds a critical issue (missing CLAUDE.md, empty skills/), report it but do not attempt auto-repair — use `references/path-hygiene-repair.md` for targeted repairs.
