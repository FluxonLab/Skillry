...
> **Reference of `mobile-app-review`.** Formerly the standalone skill `android-material-review`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: Skillry original; previously `plugins/mobile-desktop/skills/27-android-material-review/`. License: MIT (Skillry repository `LICENSE`).
> - Consolidation changes: frontmatter moved into this header; two safety lines that the hub's Safety section already states (no builds or store uploads; never print API keys) were trimmed; hub and sibling names now link to their files; body otherwise unchanged.
...
- The app is iOS-only — use `ios-hig-review` ([reference](ios-hig-review.md)).
- The review is purely about backend API calls with no UI surface.
- The codebase is React Native or Flutter — Material conventions apply differently; run the hub procedure in [../SKILL.md](../SKILL.md) (`mobile-app-review`) first to pick the right audit path.
