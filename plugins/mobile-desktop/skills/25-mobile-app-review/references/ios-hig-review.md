...
> **Reference of `mobile-app-review`.** Formerly the standalone skill `ios-hig-review`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: Skillry original; previously `plugins/mobile-desktop/skills/26-ios-hig-review/`. License: MIT (Skillry repository `LICENSE`).
> - Consolidation changes: frontmatter moved into this header; two safety lines that the hub's Safety section already states (no builds or store uploads; redact signing details) were trimmed; hub and sibling names now link to their files; body otherwise unchanged.
...
- The app is Android-only — use `android-material-review` ([reference](android-material-review.md)).
- The review is purely about business logic or backend integration — use the hub procedure in [../SKILL.md](../SKILL.md) (`mobile-app-review`).
