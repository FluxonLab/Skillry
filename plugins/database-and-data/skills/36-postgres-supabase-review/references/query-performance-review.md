...
> Absorbed into `postgres-supabase-review` in Skillry 3.0.0; formerly the standalone skill `query-performance-review`.
> Origin: Skillry original, former path `plugins/database-and-data/skills/39-query-performance-review/SKILL.md`.
...
If connection count is near the limit, review pgBouncer configuration (see step 5, connection pooling, in the main `SKILL.md`). Each serverless function invocation that opens its own `PrismaClient` is a new connection — use a singleton pattern:
