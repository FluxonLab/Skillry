...
> **Merged reference.** Former Skillry skill `env-config-hardening` (`plugins/security/skills/51-env-config-hardening/SKILL.md`, Skillry original), merged into `security-and-secrets-review` in Skillry 3.0.0.
>
> **Former description:** Use when you need to harden environment handling, config defaults, .env safety, secret loading, and redaction.
>
> **Edits on merge:** frontmatter replaced by this header; scope note and one When-not-to-use line now point inside the merged skill.

## Purpose

> **Scope note:** For committed-secret scanning and credential leak detection (git history, CORS, JWT, injection sinks), use the main `security-and-secrets-review` procedure in `SKILL.md`; this reference focuses on env var validation, config defaults, and 12-factor hardening.
...
- The project uses a secrets manager (Vault, AWS Secrets Manager, GCP Secret Manager) exclusively and the task is to configure that manager, not the application code — see `references/secrets-and-config-management.md`.
