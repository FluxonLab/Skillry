...
> **Merged reference.** Former Skillry skill `secrets-and-config-management` (`plugins/cloud-and-infrastructure/skills/335-secrets-and-config-management/SKILL.md`, Skillry original), merged into `security-and-secrets-review` in Skillry 3.0.0.
>
> **Former description:** Use when you need to review or design secret and configuration handling — secret managers (Vault, AWS Secrets Manager, SSM, GCP/Azure), rotation, runtime injection, keeping secrets out of IaC and images, sealed/external secrets, and config-vs-secret separation.
>
> **Edits on merge:** frontmatter replaced by this header; two When-not-to-use lines now name the merged skills.
...
- The task is broad application env-config typing with no secret/security dimension — use `references/env-config-hardening.md`.
- The repo has no secrets and no infrastructure that consumes them.
- A full authorization/permission model review is needed — pair with `auth-session-review` (its `references/authz-permission-review.md`).
