...
> **Merged reference.** Former Skillry skill `automation-mcp-gatekeeping` (`plugins/ai-and-agent-systems/skills/45-automation-mcp-gatekeeping/SKILL.md`, Skillry original), merged into `ai-security-review` in Skillry 3.0.0.
>
> **Former description:** Use when you need to gate external automation, MCP, Rube, Composio, credentials, scopes, and tool activation.
>
> **Edits on merge:** frontmatter replaced by this header; one checklist pointer now names the merged skill.
...
- [ ] The MCP server or connector source is trusted and has been reviewed for supply-chain risk (see `dependency-supply-chain-review` and its `references/agent-supply-chain-review.md`).
