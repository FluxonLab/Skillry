...
> **Merged reference.** Former Skillry skill `agent-governance-review` (`plugins/ai-and-agent-systems/skills/41-agent-governance-review/SKILL.md`, Skillry original), merged into `ai-security-review` in Skillry 3.0.0.
>
> **Former description:** Use when you need to review agent permissions, recursion, tool access, safety rules, and active context size.
>
> **Edits on merge:** frontmatter replaced by this header; one When-not-to-use pointer now names the merged skill.
...
- The concern is about third-party component provenance rather than runtime permission scope (use `dependency-supply-chain-review`, its `references/agent-supply-chain-review.md`).
