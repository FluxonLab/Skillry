...
> Absorbed into `game-architecture-review` in Skillry 3.0.0; formerly the standalone skill `godot-unity-unreal-triage`.
> Origin: Skillry original, former path `plugins/gaming-interactive-media/skills/34-godot-unity-unreal-triage/SKILL.md`.
...
- The issue is a frame-rate problem — use `references/game-performance-review.md` (after confirming the engine layer is not the bottleneck).
