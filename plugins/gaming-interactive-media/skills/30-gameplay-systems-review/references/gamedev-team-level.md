...
> Absorbed into `gameplay-systems-review` in Skillry 3.0.0; formerly the standalone skill `gamedev-team-level`.
> Source: [Donchitos/Claude-Code-Game-Studios](https://github.com/Donchitos/Claude-Code-Game-Studios), upstream `.claude/skills/team-level/SKILL.md`; Skillry copy formerly at `community/Donchitos-Claude-Code-Game-Studios/skills/237-gamedev-team-level/SKILL.md`.
> License: MIT, Copyright (c) 2026 Donchitos. The license text is in `references/gamedev-team-level/LICENSE`.
> Slash commands in this text use upstream names; the command map in `SKILL.md` resolves them.

Former frontmatter:

```yaml
name: gamedev-team-level
