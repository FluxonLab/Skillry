...
> Absorbed into `gameplay-systems-review` in Skillry 3.0.0; formerly the standalone skill `gamedev-map-systems`.
> Source: [Donchitos/Claude-Code-Game-Studios](https://github.com/Donchitos/Claude-Code-Game-Studios), upstream `.claude/skills/map-systems/SKILL.md`; Skillry copy formerly at `community/Donchitos-Claude-Code-Game-Studios/skills/207-gamedev-map-systems/SKILL.md`.
> License: MIT, Copyright (c) 2026 Donchitos. The license text is in `references/gamedev-map-systems/LICENSE`.
> Cross-references to skills merged in Skillry 3.0.0 now point to their reference paths.
> Path map: the upstream template path `.claude/docs/templates/systems-index.md` is bundled here as `templates/gamedev-map-systems/systems-index.md`; director gates use the shared copy `references/gamedev-brainstorm/references/director-gates.md`.
> Slash commands in this text use upstream names; the command map in `SKILL.md` resolves them.

Former frontmatter:

```yaml
name: gamedev-map-systems
...
See `references/gamedev-brainstorm/references/director-gates.md` for the full check pattern.
...
**After dependency mapping is approved, spawn `technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`references/gamedev-brainstorm/references/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `producer` via Task using gate PR-SCOPE (`references/gamedev-brainstorm/references/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via Task using gate CD-SYSTEMS (`references/gamedev-brainstorm/references/director-gates.md`).**
