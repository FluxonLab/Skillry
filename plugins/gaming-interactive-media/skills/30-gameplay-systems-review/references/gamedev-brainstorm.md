...
> Absorbed into `gameplay-systems-review` in Skillry 3.0.0; formerly the standalone skill `gamedev-brainstorm`.
> Source: [Donchitos/Claude-Code-Game-Studios](https://github.com/Donchitos/Claude-Code-Game-Studios), upstream `.claude/skills/brainstorm/SKILL.md`; Skillry copy formerly at `community/Donchitos-Claude-Code-Game-Studios/skills/186-gamedev-brainstorm/SKILL.md`.
> License: MIT, Copyright (c) 2026 Donchitos. The license text is in `references/gamedev-brainstorm/LICENSE`.
> Cross-references to skills merged in Skillry 3.0.0 now point to their reference paths.
> Path map: the upstream template path `.claude/docs/templates/game-concept.md` is bundled here as `templates/gamedev-brainstorm/game-concept.md`.
> Slash commands in this text use upstream names; the command map in `SKILL.md` resolves them.

Former frontmatter:

```yaml
name: gamedev-brainstorm
...
- **`creative-director`** — gate **CD-PILLARS** (`references/gamedev-brainstorm/references/director-gates.md`)
 Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`references/gamedev-brainstorm/references/director-gates.md`)
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`references/gamedev-brainstorm/references/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`references/gamedev-brainstorm/references/director-gates.md`).**
