...
> **Former skill `codex-final-execution-prompt`:** Use when you need to write strict final execution prompts for Codex, Claude, Copilot, or other coding agents.
> **Provenance:** Skillry original, moved from `plugins/product-docs-and-research/skills/64-codex-final-execution-prompt/SKILL.md`. Changes: frontmatter turned into this header; references to skills merged into this hub now link to their reference files.
...
- A plan exists (from `implementation-plan`) and now needs translating into a strict agent-executable format with exact file/change pairs and verification commands.
...
- A narrower skill already covers the need: `implementation-plan` for human-facing specs and handoff packs (its former `md-spec-generator` part) and for the planning phase that precedes prompt-writing.
