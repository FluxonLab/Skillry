...
> Absorbed into `cost-control-token-review` in Skillry 3.0.0; formerly the standalone skill `llm-api-cost-optimization`.
> Origin: Skillry original, former path `plugins/performance-and-cost/skills/325-llm-api-cost-optimization/SKILL.md`.
...
- The driver is GPU/self-hosted compute, not per-token API billing — that is cloud spend review territory (`references/cloud-spend-review.md`).
