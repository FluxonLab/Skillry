...
> **Reference of `seo-content-ops-review`.** Formerly the standalone skill `seo-cluster`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: third-party, [AgriciDaniel/claude-seo](https://github.com/AgriciDaniel/claude-seo) `skills/seo-cluster/SKILL.md` at commit 2384fdd9429696021d143006a722c7cf1aa76d43 (byte-identical; sidecars matched at the commits recorded for the lock); imported from a local unmanaged install.
> - License: MIT, Copyright (c) 2026 agricidaniel (upstream root `LICENSE` checked 2026-09-19, blob 94fac746, same at the pinned commits); full text at [seo-cluster/LICENSE](seo-cluster/LICENSE).
> - Original author per metadata: Lutfiya Miller. The optional claude-blog execution step refers to a separate upstream plugin.
...
> - Consolidation changes: frontmatter moved into this header; paths to its own sidecar files now point to the moved, byte-identical copies; body otherwise unchanged.
...
This is the core differentiator. Load `references/seo-cluster/references/serp-overlap-methodology.md` for
the full algorithm.
...
Load `references/seo-cluster/references/hub-spoke-architecture.md` for full specifications.
...
Generate `cluster-map.html` using the template at `templates/seo-cluster/cluster-map.html`.
...
1. Load `references/seo-cluster/references/execution-workflow.md` for the full algorithm
