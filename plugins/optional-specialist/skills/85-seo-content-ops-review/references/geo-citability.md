...
> **Reference of `seo-content-ops-review`.** Formerly the standalone skill `geo-citability`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: third-party, [TheSmokeDev/geo-skills](https://github.com/TheSmokeDev/geo-skills) `skills/geo-citability/SKILL.md` at commit 7f3ec21a065a8af39460bcace8bcd723b7d52145 (byte-identical); imported from a local unmanaged install.
> - License: MIT, Copyright (c) 2026 SmokeDev (upstream `LICENSE` checked 2026-09-19, blob 81ad5d5e); copy kept at [geo-citability/LICENSE](geo-citability/LICENSE).
> - Lineage: this text is a close revision of `skills/geo-citability/SKILL.md` in [zubair-trabzada/geo-seo-claude](https://github.com/zubair-trabzada/geo-seo-claude) (MIT, Copyright (c) 2026 Zubair Trabzada, `LICENSE` blob 29ee5a8f); that notice is kept at [geo-citability/LICENSE-geo-seo-claude](geo-citability/LICENSE-geo-seo-claude).
> - Family note: Sibling GEO skills folded into this hub: geo-schema, geo-citability, geo-crawlers, geo-ai-index-access, geo-fanout (see the routing table in [../SKILL.md](../SKILL.md)); others named below (for example `/geo-measurement`, `/geo-youtube`, `geo-audit`) are upstream-only.
> - Consolidation changes: frontmatter moved into this header (it records the upstream tool allowlist); body unchanged.
