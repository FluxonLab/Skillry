> **Reference of [`technical-writing-review`](../SKILL.md)** (merged in Skillry 3.0.0).
> **Former skill `api-reference-docs`:** Use when you need to produce or audit API reference documentation from code or an OpenAPI spec — complete endpoint/parameter coverage, runnable request/response examples, an error/status-code table, authentication notes, and a clear versioning and deprecation policy.
> **Provenance:** Skillry original, moved from `plugins/documentation-and-tech-writing/skills/302-api-reference-docs/SKILL.md`. Changes: frontmatter turned into this header; references to skills merged into this hub now link to their reference files.
...
- The audience needs a goal-oriented walkthrough, not an exhaustive parameter list (use [tutorial-and-how-to-writing](tutorial-and-how-to-writing.md)).
- The API is purely internal, single-consumer, and fully described by inline types that the one caller already reads.
- You are documenting a release's user-facing changes (use [changelog-and-release-notes](changelog-and-release-notes.md)).
