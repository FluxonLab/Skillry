> **Reference of [`business-model-review`](../SKILL.md)** (merged in Skillry 3.0.0).
> **Former skill `cold-start-problem`:** Start and scale networked products using Andrew Chen's "The Cold Start Problem" framework for network effects. Use when the user mentions "network effects", "chicken and egg", "cold start", "two-sided marketplace", "atomic network", "hard side", "liquidity", "critical mass", "invite-only launch", "how do I get my first users", or "the marketplace has no buyers or sellers". Also trigger when launching a marketplace, social, or collaboration product that is worthless without other users, deciding launch sequencing and seeding tactics, or diagnosing stalled network growth at scale. Covers the five stages: cold start, tipping point, escape velocity, hitting the ceiling, and the moat. For word-of-mouth virality, see contagious. For habit-driven retention, see hooked-ux.
> **Provenance:** third-party, from [wondelai/skills](https://github.com/wondelai/skills) at commit `c172996495be` (`cold-start-problem/SKILL.md`, sha256 `6ee3a73a0ab3c0e0`), imported 2026-09-19. Copyright (c) 2025 Wondel.ai sp. z o.o. MIT License: [cold-start-problem/LICENSE](cold-start-problem/LICENSE). Changes: frontmatter turned into this header. Paths `references/...` in the text were re-rooted to `cold-start-problem/...` (10 places); the files under `cold-start-problem/` are byte-identical to upstream.
> **Note:** `contagious` and `hooked-ux`, named in the original trigger, are upstream wondelai skills that Skillry does not ship. Book links in the text are upstream links, kept verbatim.
> **Upstream frontmatter besides name and description:**
>
> ```yaml
> license: MIT
> metadata:
...
See [cold-start-problem/references/case-studies.md](cold-start-problem/references/case-studies.md) for three end-to-end worked scenarios — a B2B tool finding its atomic network, a services marketplace seeding one city, a social app recovering from a big-bang launch — when you want a full example to model a plan on.
...
See [cold-start-problem/references/atomic-networks.md](cold-start-problem/references/atomic-networks.md) when scoping the first launch — it has the 5-step minimum-size derivation, the actor/action/response/time magic-moment template, instrumentation and zero-rate steps, honest-flintstoning rules, single-player fallbacks, and a launch checklist.
...
See [cold-start-problem/references/hard-side.md](cold-start-problem/references/hard-side.md) when designing supply-side acquisition and economics — it maps money/status/utility motivations to product investments and details three named playbooks (tools-first, content-first, subsidies).
...
See [cold-start-problem/references/tipping-playbooks.md](cold-start-problem/references/tipping-playbooks.md) when planning network #2 onward — invite-only and referral-tree mechanics, paid-launch and supply pre-commitment tactics, market selection, anti-patterns, and the liquidity metrics to gate on.
...
See [cold-start-problem/references/scale-ceiling-moat.md](cold-start-problem/references/scale-ceiling-moat.md) when growth stalls or a rival appears — it runs the three forces as growth workstreams, diagnoses which ceiling hit first, and details quality interventions and cherry-picking defense at scale.
