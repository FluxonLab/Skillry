...
> **Former skill `pricing-packaging-review`:** Use when you need to review pricing tiers, packaging, feature gates, trials, and conversion paths.
> **Provenance:** Skillry original, moved from `plugins/product-docs-and-research/skills/68-pricing-packaging-review/SKILL.md`. Changes: frontmatter turned into this header; references to skills merged into this hub now link to their reference files.
...
- The underlying business model (revenue type, unit economics) has not been validated — use the main `business-model-review` procedure ([../SKILL.md](../SKILL.md)) first to confirm the model is viable before optimising its packaging.
