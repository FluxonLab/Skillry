...
> **Reference of `accessibility-audit`.** Formerly the standalone skill `accessibility`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: third-party, [addyosmani/web-quality-skills](https://github.com/addyosmani/web-quality-skills) `skills/accessibility/` at commit afa8da942115f2961fdbfa80807ea0b232ff6c00 (all three files byte-identical; SKILL.md blob matched upstream); imported from a local unmanaged install. The same upstream already supplies Skillry's `community/addyosmani-web-quality-skills/`.
> - License: MIT, Copyright (c) 2026 Addy Osmani (upstream `LICENSE` checked 2026-09-19, blob 90715bb4); copy kept at [accessibility/LICENSE](accessibility/LICENSE).
> - Files: the pattern and WCAG sidecars are byte-identical upstream copies under `references/accessibility/references/`; a `../SKILL.md` link inside them means this file.
> - Consolidation changes: frontmatter moved into this header; links to its sidecar files now point to the moved copies; the link to the sibling upstream skill `web-quality-audit` (not vendored) now points to the upstream repository; body otherwise unchanged.
...
**No keyboard traps.** Users must be able to Tab into and out of every component. Use the [modal focus trap pattern](accessibility/references/A11Y-PATTERNS.md#modal-focus-trap) for dialogs—the native `<dialog>` element handles this automatically.
...
Provide a skip link so keyboard users can bypass repetitive navigation. See the [skip link pattern](accessibility/references/A11Y-PATTERNS.md#skip-link) for full markup and styles.
...
Any action that requires dragging must have a single-pointer alternative (e.g., buttons, inputs). See the [dragging movements pattern](accessibility/references/A11Y-PATTERNS.md#dragging-movements) for a sortable-list example.
...
Every input needs a programmatically associated label. See the [form labels pattern](accessibility/references/A11Y-PATTERNS.md#form-labels) for explicit, implicit, and instructional examples.

### Error handling (3.3.1, 3.3.3)

Announce errors to screen readers with `role="alert"` or `aria-live`, set `aria-invalid="true"` on invalid fields, and focus the first error on submit. See the [error handling pattern](accessibility/references/A11Y-PATTERNS.md#error-handling) for full markup and JS.
...
**When ARIA is needed,** use the correct roles and states. See the [ARIA tabs pattern](accessibility/references/A11Y-PATTERNS.md#aria-tabs) for a complete tablist example.

### Live regions (4.1.3)

Use `aria-live` regions to announce dynamic content changes without moving focus. See the [live regions pattern](accessibility/references/A11Y-PATTERNS.md#live-regions-and-notifications) for markup and a `showNotification()` helper.
...
See the [screen reader commands reference](accessibility/references/A11Y-PATTERNS.md#screen-reader-commands) for VoiceOver and NVDA shortcuts.
...
- [Web Quality Audit](https://github.com/addyosmani/web-quality-skills/tree/main/skills/web-quality-audit) (upstream skill, not vendored)
- [WCAG criteria reference](accessibility/references/WCAG.md)
- [Accessibility code patterns](accessibility/references/A11Y-PATTERNS.md)
