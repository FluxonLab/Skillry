...
> **Reference of `next-app-router-rsc-review`.** Formerly the standalone skill `nextjs-app-router-patterns`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: third-party, [wshobson/agents](https://github.com/wshobson/agents); previously `community/wshobson-agents/skills/nextjs-app-router-patterns/`.
> - License: MIT, Copyright (c) 2024 Seth Hobson (upstream `LICENSE` checked 2026-09-19, blob 326f0a55, identical to the vendored copy); copy kept at [nextjs-app-router-patterns/LICENSE](nextjs-app-router-patterns/LICENSE).
> - Scope note: this reference is the build-side pattern catalog (it targets Next.js 14+); the hub's review procedure and its Next.js 15 caching defaults take precedence when the two differ.
> - Earlier Skillry import: vendored from `wshobson-agents` on 2026-05-31.
> - Consolidation changes: frontmatter moved into this header; body unchanged.
