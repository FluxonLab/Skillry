...
> **Reference of `landing-page-conversion-review`.** Formerly the standalone skill `onboarding`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: third-party, [coreyhaines31/marketingskills](https://github.com/coreyhaines31/marketingskills) `skills/onboarding/` at commit 5b2c0007766c6a1cf1d53fd8fc73e979e0821022 (every imported file byte-identical to upstream); imported from a local unmanaged install.
> - License: MIT, Copyright (c) 2025 Corey Haines (upstream `LICENSE` checked 2026-09-19, blob 7c48dd63); copy kept at [onboarding/LICENSE](onboarding/LICENSE).
> - Files: sidecars (eval fixtures for maintainers, supporting references) are byte-identical upstream copies under `references/onboarding/`.
> - Note: `.agents/product-marketing.md` is a project context file the user may keep, not a Skillry file. Other marketingskills siblings named below (emails, paywalls, ab-testing) are not part of this hub.
> - Consolidation changes: frontmatter moved into this header; links to its own sidecar files now point to the moved copies; names of skills merged into the same hub link to their references; body otherwise unchanged.
...
Remove every step between signup and experiencing core value. Design the **Minimum Path to Value (MPTV)** — the least number of steps to experience enough value to make a confident decision (see [references/onboarding/references/minimum-path-to-value.md](onboarding/references/minimum-path-to-value.md)).
...
Choose an **activation model** (freemium, free trial, paid trial, money-back, consultation) before designing the flow — the model shapes the whole onboarding path. See [references/onboarding/references/activation-models.md](onboarding/references/activation-models.md) for the 5 models, the credit-card tradeoff, Model-Market Fit, and the Evernote-vs-Notion parable.
...
**For comprehensive experiment ideas**: See [references/onboarding/references/experiments.md](onboarding/references/experiments.md)

---

## References

- **[references/onboarding/references/minimum-path-to-value.md](onboarding/references/minimum-path-to-value.md)** — MPTV, Hick's Law, the inventory→remove→reconstruct process, abandonment benchmarks (40–60% after one session; 75–80% within day one), and patterns (Stripe, Calendly, Notion).
- **[references/onboarding/references/activation-models.md](onboarding/references/activation-models.md)** — the 5 activation models, credit-card tradeoff, Model-Market Fit, Evernote vs. Notion.
- **[references/onboarding/references/experiments.md](onboarding/references/experiments.md)** — comprehensive A/B test and experiment ideas.
...
- **signup** ([reference](signup.md)): For optimizing the signup before onboarding
