...
> **Reference of `web-design-review`.** Formerly the standalone skill `design-system-review`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: Skillry original; previously `plugins/frontend-web-design/skills/19-design-system-review/`. License: MIT (Skillry repository `LICENSE`).
> - Consolidation changes: frontmatter moved into this header; names of skills merged into the same hub now link to their references; body otherwise unchanged.
...
- The request is about a single page's layout, not system-level patterns — use the hub procedure in [../SKILL.md](../SKILL.md) (`web-design-review`).
- The request is about final pixel polish of one feature — use `visual-polish-pass` ([reference](visual-polish-pass.md)).
- The request is about per-component consistency against the system — use `ui-consistency-review` ([reference](ui-consistency-review.md)).
