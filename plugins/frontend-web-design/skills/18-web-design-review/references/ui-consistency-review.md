...
> **Reference of `web-design-review`.** Formerly the standalone skill `ui-consistency-review`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: Skillry original; previously `plugins/frontend-web-design/skills/17-ui-consistency-review/`. License: MIT (Skillry repository `LICENSE`).
> - Consolidation changes: frontmatter moved into this header; generic When-not-to-use boilerplate that the hub already covers was trimmed; names of skills merged into the same hub now link to their references; body otherwise unchanged.
...
- The page has structural layout breakage (wrong hierarchy, broken mobile). Fix structure with the hub procedure in [../SKILL.md](../SKILL.md) (`web-design-review`) or `responsive-layout-review` ([reference](responsive-layout-review.md)) first, then run consistency.
- A narrower skill already covers the exact need: `design-system-review` ([reference](design-system-review.md)) for token architecture and duplicate primitives, `accessibility-audit` for a full WCAG pass, `visual-polish-pass` ([reference](visual-polish-pass.md)) for final pixel finish.
