...
> **Merged reference.** Former Skillry skill `browser-first-local-app` (`plugins/runtime-and-local-app/skills/07-browser-first-local-app/SKILL.md`, Skillry original), merged into `local-launcher-shortcuts` in Skillry 3.0.0.
>
> **Former description:** Use when you need to prefer a local service plus browser UI over desktop runtime complexity unless a native shell is explicitly required.
>
> **Edits on merge:** frontmatter replaced by this header; generic When-not-to-use lines that repeated the hub's SKILL.md removed.
