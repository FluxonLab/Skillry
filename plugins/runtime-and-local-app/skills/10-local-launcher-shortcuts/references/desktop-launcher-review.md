...
> **Merged reference.** Former Skillry skill `desktop-launcher-review` (`plugins/mobile-desktop/skills/28-desktop-launcher-review/SKILL.md`, Skillry original), merged into `local-launcher-shortcuts` in Skillry 3.0.0.
>
> **Former description:** Use when you need to review local desktop launchers, shortcuts, shell scripts, app wrappers, logging, and update safety.
>
> **Edits on merge:** frontmatter replaced by this header; generic When-not-to-use lines that repeated the hub's SKILL.md removed; the Electron security pointer now names its new location.
...
- The concern is renderer or runtime security rather than launch correctness — use `security-and-secrets-review` (its `references/electron-app-security-review.md`) for `contextIsolation`, CSP, and IPC hardening.
