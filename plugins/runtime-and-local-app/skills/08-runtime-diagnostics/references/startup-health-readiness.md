...
> **Merged reference.** Former Skillry skill `startup-health-readiness` (`plugins/runtime-and-local-app/skills/09-startup-health-readiness/SKILL.md`, Skillry original), merged into `runtime-diagnostics` in Skillry 3.0.0.
>
> **Former description:** Use when you need to add or verify readiness gates, health checks, and startup sequencing.
>
> **Edits on merge:** frontmatter replaced by this header; one When-not-to-use pointer now points at the hub's SKILL.md.
...
- The service already has tested, working health endpoints and you just need to debug a transient issue — use the main `runtime-diagnostics` procedure in `SKILL.md` instead.
