> **Merged skill:** `python-anti-patterns` is part of `python-project-review` since Skillry 3.0.0 (routing table in `SKILL.md`).
> **Source:** [wshobson/agents](https://github.com/wshobson/agents) `plugins/python-development/skills/python-anti-patterns/SKILL.md`, redistributed in Skillry since 2026-05-31; formerly `community/wshobson-agents/skills/python-anti-patterns/SKILL.md`.
> **License:** MIT, Copyright (c) 2024 Seth Hobson. Full text: [`references/python-anti-patterns/LICENSE`](python-anti-patterns/LICENSE).
> **Changes:** YAML frontmatter replaced by this header (and the former vendor-import note); cross-reference to a sibling skill re-pointed to its reference file.
> **Former description:** Use this skill when reviewing Python code for common anti-patterns to avoid. Use as a checklist when reviewing code, before finalizing implementations, or when debugging issues that might stem from known bad practices.
...
**Note:** This skill focuses on what to avoid. For guidance on positive patterns and architecture, see [`references/python-design-patterns.md`](python-design-patterns.md).
