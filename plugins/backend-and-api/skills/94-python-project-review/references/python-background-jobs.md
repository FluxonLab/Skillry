> **Merged skill:** `python-background-jobs` is part of `python-project-review` since Skillry 3.0.0 (routing table in `SKILL.md`).
> **Source:** [wshobson/agents](https://github.com/wshobson/agents) `plugins/python-development/skills/python-background-jobs/SKILL.md`, redistributed in Skillry since 2026-05-31; formerly `community/wshobson-agents/skills/python-background-jobs/SKILL.md`.
> **License:** MIT, Copyright (c) 2024 Seth Hobson. Full text: [`references/python-background-jobs/LICENSE`](python-background-jobs/LICENSE).
> **Changes:** YAML frontmatter replaced by this header (and the former vendor-import note); body unchanged.
> **Former description:** Python background job patterns including task queues, workers, and event-driven architecture. Use when implementing async task processing, job queues, long-running operations, or decoupling work from request/response cycles.
