> **Merged skill:** `integration-boundary-review` is part of `backend-implementation-review` since Skillry 3.0.0 (routing table in `SKILL.md`).
> **Source:** Skillry original, formerly `plugins/backend-and-api/skills/15-integration-boundary-review/SKILL.md`.
> **Changes:** YAML frontmatter replaced by this header; self-reference to the canonical skill re-pointed to SKILL.md.
> **Former description:** Use when you need to review third-party service, webhook, queue, and cross-system integration boundaries.
...
## When not to use
- The integration is internal (same database, same process) — that is the main `backend-implementation-review` procedure in SKILL.md.
