...
> **Merged reference.** Former Skillry skill `authz-permission-review` (`plugins/security/skills/49-authz-permission-review/SKILL.md`, Skillry original), merged into `auth-session-review` in Skillry 3.0.0.
>
> **Former description:** Use when you need to review authorization, roles, row-level access, tenancy, privilege checks, and denied paths.
>
> **Edits on merge:** frontmatter replaced by this header; two When-not-to-use lines now point at the merged skill names.
...
- The task is purely about authentication (login, token issuance, MFA) — use the main `auth-session-review` procedure in `SKILL.md`.
...
- The concern is secrets or CORS configuration — `security-and-secrets-review` covers those; this reference focuses on authorization logic.
