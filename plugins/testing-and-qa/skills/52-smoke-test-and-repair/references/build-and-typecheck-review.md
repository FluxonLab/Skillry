...
> **Provenance.** Absorbed into `smoke-test-and-repair` in Skillry 3.0.0. Formerly the standalone skill `build-and-typecheck-review` (`plugins/devops-and-release/skills/59-build-and-typecheck-review/SKILL.md`). Origin: Skillry original, MIT (repository `LICENSE`).
>
> **Changes:** frontmatter converted to this header; cross-references to merged skills updated.
>
> **Former trigger:** Use when you need to review build, typecheck, compile, bundling, and static validation commands.
...
- Build issues are in a Docker layer or deployment step — use `release-readiness-check` (its `references/deployment-preflight-review.md`).
