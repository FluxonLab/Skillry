...
> **Reference of `uat-acceptance-review`.** Formerly the standalone skill `designing-functional-tests`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: third-party, [jaktestowac/awesome-copilot-for-testers](https://github.com/jaktestowac/awesome-copilot-for-testers); previously `community/jaktestowac-awesome-copilot-for-testers/skills/designing-functional-tests/`.
> - License: MIT (upstream `LICENSE` checked 2026-09-19, blob 89bc5e96, identical to the vendored copy); copy kept at [designing-functional-tests/LICENSE](designing-functional-tests/LICENSE).
> - Note: the `./resources/*.md` templates named in the body were not part of the earlier Skillry import (get them from the upstream repository, or use the structure described inline).
> - Earlier Skillry import: vendored from `jaktestowac-awesome-copilot-for-testers` on 2026-05-31.
> - Consolidation changes: frontmatter moved into this header; sibling skill names now link to their references; body otherwise unchanged.
...
- send requirements-heavy work into `requirements-test-coverage-mapper` ([reference](requirements-test-coverage-mapper.md)) when traceability matters more than step-by-step execution
...
- `requirements-test-coverage-mapper` ([reference](requirements-test-coverage-mapper.md)) - when traceability to PRD, stories, or AC is the main deliverable
