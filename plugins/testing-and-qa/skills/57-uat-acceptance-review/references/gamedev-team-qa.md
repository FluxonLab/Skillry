# Game QA team cycle (gamedev-team-qa)

> **Reference of `uat-acceptance-review`.** Formerly the standalone skill `gamedev-team-qa`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: third-party, [Donchitos/Claude-Code-Game-Studios](https://github.com/Donchitos/Claude-Code-Game-Studios); previously `community/Donchitos-Claude-Code-Game-Studios/skills/241-gamedev-team-qa/`.
> - License: MIT (upstream `LICENSE` checked 2026-09-19, blob 416f2e52, identical to the vendored copy); copy kept at [gamedev-team-qa/LICENSE](gamedev-team-qa/LICENSE).
> - Note: the `qa-lead` / `qa-tester` subagents and slash commands (`/team-qa`, `/gate-check`, `/create-stories`, `/architecture-decision`) belong to the Game Studios agent set; the former frontmatter below records the tool allowlist and lead agent this workflow expects.
> - Consolidation changes: frontmatter moved into this header (a title was added because the original body had none); body unchanged.

<details><summary>Former frontmatter (kept for provenance)</summary>

```yaml
name: gamedev-team-qa
