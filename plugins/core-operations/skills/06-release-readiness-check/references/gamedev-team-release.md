...
> **Provenance.** Absorbed into `release-readiness-check` in Skillry 3.0.0. Formerly the standalone skill `gamedev-team-release` (`community/Donchitos-Claude-Code-Game-Studios/skills/242-gamedev-team-release/SKILL.md`, community source `Donchitos-Claude-Code-Game-Studios`).
>
> **Upstream:** https://github.com/Donchitos/Claude-Code-Game-Studios `.claude/skills/team-release/SKILL.md`. **License:** MIT, Copyright (c) 2026 Donchitos; full text in `references/gamedev-team-release/LICENSE`.
>
> **Changes:** frontmatter converted to this header; upstream slash commands that map to merged Skillry skills annotated with their reference (/release-checklist, /retrospective); other slash commands name upstream Claude Code Game Studios commands that Skillry does not ship; title heading added (the source file has none).
>
> **Former trigger:** Orchestrate the release team: coordinates release-manager, qa-lead, devops-engineer, and producer to execute a release from candidate to deployment.
>
...
- Generate the release checklist using `/release-checklist` [Skillry: this skill → `references/gamedev-release-checklist.md`]
...
- Run `/retrospective` [Skillry: `implementation-plan` → `references/gamedev-retrospective.md`] if significant issues occurred during the release.
