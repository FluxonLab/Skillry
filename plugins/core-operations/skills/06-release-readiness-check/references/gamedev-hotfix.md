...
> **Provenance.** Absorbed into `release-readiness-check` in Skillry 3.0.0. Formerly the standalone skill `gamedev-hotfix` (`community/Donchitos-Claude-Code-Game-Studios/skills/204-gamedev-hotfix/SKILL.md`, community source `Donchitos-Claude-Code-Game-Studios`).
>
> **Upstream:** https://github.com/Donchitos/Claude-Code-Game-Studios `.claude/skills/hotfix/SKILL.md`. **License:** MIT, Copyright (c) 2026 Donchitos; full text in `references/gamedev-hotfix/LICENSE`.
>
> **Changes:** frontmatter converted to this header; cross-references to merged skills updated; upstream slash commands that map to merged Skillry skills annotated with their reference (/retrospective, /team-qa); other slash commands name upstream Claude Code Game Studios commands that Skillry does not ship; title heading added (the source file has none).
>
> **Former trigger:** Emergency fix workflow that bypasses normal sprint processes with a full audit trail. Creates hotfix branch, tracks approvals, and ensures the fix is backported correctly.
>
...
> **Explicit invocation only**: Follow this reference only when the user explicitly asks for a hotfix (upstream command /hotfix). Do not auto-invoke based on context matching.
...
- **Targeted QA pass required** — run `/team-qa [affected-system]` [Skillry: `uat-acceptance-review` → `references/gamedev-team-qa.md`] scoped to the changed system only. If QA returns APPROVED or APPROVED WITH CONDITIONS, proceed to Phase 6.
- **Full QA required** — S1 fixes that touch core systems may require a full `/team-qa sprint` [Skillry: `uat-acceptance-review` → `references/gamedev-team-qa.md`]. This delays deployment but prevents a bad patch.
...
Schedule a post-incident review within 48 hours using `/retrospective hotfix` [Skillry: `implementation-plan` → `references/gamedev-retrospective.md`].
