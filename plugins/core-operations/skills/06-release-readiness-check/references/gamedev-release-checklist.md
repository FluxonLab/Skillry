# Gamedev Release Checklist

> **Provenance.** Absorbed into `release-readiness-check` in Skillry 3.0.0. Formerly the standalone skill `gamedev-release-checklist` (`community/Donchitos-Claude-Code-Game-Studios/skills/219-gamedev-release-checklist/SKILL.md`, community source `Donchitos-Claude-Code-Game-Studios`).
>
> **Upstream:** https://github.com/Donchitos/Claude-Code-Game-Studios `.claude/skills/release-checklist/SKILL.md`. **License:** MIT, Copyright (c) 2026 Donchitos; full text in `references/gamedev-release-checklist/LICENSE`.
>
> **Changes:** frontmatter converted to this header; cross-references to merged skills updated; upstream slash commands that map to merged Skillry skills annotated with their reference (/team-release); other slash commands name upstream Claude Code Game Studios commands that Skillry does not ship; title heading added (the source file has none).
>
> **Former trigger:** Generates a comprehensive pre-release validation checklist covering build verification, certification requirements, store metadata, and launch readiness.
>
...
> **Explicit invocation only**: Follow this reference only when the user explicitly asks for a release checklist (upstream command /release-checklist). Do not auto-invoke based on context matching.
...
- Coordinate final sign-offs via `/team-release` [Skillry: this skill → `references/gamedev-team-release.md`].
