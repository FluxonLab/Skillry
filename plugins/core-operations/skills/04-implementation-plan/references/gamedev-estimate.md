...
> **Provenance.** Absorbed into `implementation-plan` in Skillry 3.0.0. Formerly the standalone skill `gamedev-estimate` (`community/Donchitos-Claude-Code-Game-Studios/skills/201-gamedev-estimate/SKILL.md`, community source `Donchitos-Claude-Code-Game-Studios`).
>
> **Upstream:** https://github.com/Donchitos/Claude-Code-Game-Studios `.claude/skills/estimate/SKILL.md`. **License:** MIT, Copyright (c) 2026 Donchitos; full text in `references/gamedev-estimate/LICENSE`.
>
> **Changes:** frontmatter converted to this header; upstream slash commands that map to merged Skillry skills annotated with their reference (/prototype); other slash commands name upstream Claude Code Game Studios commands that Skillry does not ship; title heading added (the source file has none).
>
> **Former trigger:** Estimates task effort by analyzing complexity, dependencies, historical velocity, and risk factors. Produces a structured estimate with confidence levels.
>
...
- If confidence is Low: recommend a time-boxed spike (`/prototype` [Skillry: `gameplay-systems-review` → `references/gamedev-prototype.md`]) before committing.
