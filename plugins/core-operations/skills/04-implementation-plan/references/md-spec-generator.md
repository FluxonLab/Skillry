...
> **Provenance.** Absorbed into `implementation-plan` in Skillry 3.0.0. Formerly the standalone skill `md-spec-generator` (`plugins/product-docs-and-research/skills/63-md-spec-generator/SKILL.md`). Origin: Skillry original, MIT (repository `LICENSE`).
>
> **Changes:** frontmatter converted to this header; 2 generic When-not-to-use boilerplate line(s) that repeat the canonical removed; cross-references to merged skills updated.
>
> **Former trigger:** Use when you need to create clean markdown specifications, implementation docs, repair reports, and handoff packs.
...
- A narrower skill already covers the need: `prompt-systems-review` (its `references/codex-final-execution-prompt.md`) for agent-executable prompts, `architecture-review` (its `references/adr-generator.md`) for architecture decision records, `technical-writing-review` for prose-quality review.
