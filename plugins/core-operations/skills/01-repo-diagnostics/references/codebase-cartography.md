...
> **Provenance.** Absorbed into `repo-diagnostics` in Skillry 3.0.0. Formerly the standalone skill `codebase-cartography` (`plugins/core-operations/skills/02-codebase-cartography/SKILL.md`). Origin: Skillry original, MIT (repository `LICENSE`).
>
> **Changes:** frontmatter converted to this header; cross-references to merged skills updated.
>
> **Former trigger:** Use when you need to map module layout, runtime boundaries, data flow, ownership zones, and high-risk files.
...
- You need a runnability or health baseline rather than a structural map — use the main `repo-diagnostics` procedure (SKILL.md).
