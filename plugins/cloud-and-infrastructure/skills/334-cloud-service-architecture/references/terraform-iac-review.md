...
> **Merged reference.** Former Skillry skill `terraform-iac-review` (`plugins/cloud-and-infrastructure/skills/331-terraform-iac-review/SKILL.md`, Skillry original), merged into `cloud-service-architecture` in Skillry 3.0.0.
>
> **Former description:** Use when you need to review Terraform or OpenTofu infrastructure-as-code for remote state and locking correctness, module structure, drift, plan safety, hardcoded secrets, provider pinning, and least-privilege IAM before any apply.
>
> **Edits on merge:** frontmatter replaced by this header; one When-not-to-use line now names the pipeline skill.
...
- The change is CI/CD pipeline wiring only (runner config, workflow YAML) — that is a pipeline review (`ci-cd-pipeline-review`), not IaC.
