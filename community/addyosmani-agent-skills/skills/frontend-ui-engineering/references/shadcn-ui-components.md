...
> **Reference of `frontend-ui-engineering`.** Formerly the standalone skill `shadcn-ui-components`; the routing table in [../SKILL.md](../SKILL.md) sends that name and its topics here.
>
> - Origin: Skillry original (FluxonLab); previously `plugins/frontend-web-design/skills/256-shadcn-ui-components/`.
> - License: MIT, Copyright (c) 2026 FluxonLab (the Skillry repository `LICENSE`); a copy sits at [shadcn-ui-components/LICENSE](shadcn-ui-components/LICENSE) because this hub lives in a third-party source folder.
> - Consolidation changes: frontmatter moved into this header; body unchanged (its When-not-to-use and safety notes are shadcn-specific, not hub boilerplate).
