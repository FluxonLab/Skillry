...
Repository-scoped contributor instructions for OpenAI Codex and other agents working in Skillry.

This file is not a general project template, is not portable installer payload, and must not be
copied into unrelated repositories. CLAUDE.md is an equivalent entry point for Claude Code. Keep
their shared rules aligned; neither file creates a higher policy layer.
